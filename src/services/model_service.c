...
const char* get_mock_prediction(float amplitude) {
    // Mock prediction based on amplitude
    if (amplitude > 800) {
        return "Faulty";
    }
    return "Normal";
...
int process_full_second(prediction_buffer_t *buffer)
{
    ...
    const char* prediction = get_mock_prediction(average);
    ...
    snprintf(payload, sizeof(payload),
            ...
            "\"prediction\":\"%s\","
            "\"confidence\":%.2f}",
            now, average, prediction, confidence);
